...
export default function Index() {
  usePageMeta({
    title: "AI Rådgivning",
    description:
      "Vi hjælper mellemstore danske virksomheder med at finde, prioritere og indføre AI dér hvor det skaber værdi. Rådgivning, uddannelse og udvikling — nede på jorden.",
    path: "/",
  });
  ...
  const onSubscribe = (e: React.FormEvent) => {
    e.preventDefault();
    // TODO: wire to MailerLite (konto 1571946).
    if (!consent) {
  ...
  return (
    <Layout>
      ...
      <section className="relative overflow-hidden">
        <img src={markDark} alt="" aria-hidden className="pointer-events-none absolute -left-16 -top-10 w-72 opacity-[0.06] md:w-96" />
        <div className="container grid items-center gap-10 py-16 md:grid-cols-2 md:gap-12 md:py-24">
          <Reveal>
            ...
            <p className="lead mt-5 max-w-prose">
              Vi hjælper jer med at finde, prioritere og indføre AI dér, hvor det skaber værdi —
              afdeling for afdeling. Nede på jorden og uden hype.
            </p>
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <Button asChild size="lg">
                <Link to="/book">Book et uforpligtende møde <ArrowRight className="h-4 w-4" /></Link>
              </Button>
              <Button asChild variant="outline" size="lg">
                <Link to="/ydelser">Se hvad vi gør</Link>
              </Button>
            </div>
          ...
          <Reveal delay={120} className="relative">
            ...
            <div className="overflow-hidden rounded-2xl border border-border shadow-sm clip-slope-b">
              <img src={jacob} alt="Jacob Dalhoff, AI-rådgiver hos AI Rådgivning" className="aspect-[4/5] w-full object-cover" width={933} height={1166} loading="eager" />
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <section aria-label="Troværdighed" className="border-y border-border bg-muted/40">
        <div className="container flex flex-col items-center justify-between gap-4 py-5 text-sm text-muted-foreground sm:flex-row">
          ...
          <ul className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
            {MEMBERSHIPS.map((m) => (
              <li key={m} className="font-display font-semibold tracking-tight text-foreground/70">{m}</li>
            ))}
        ...
        <div className="container">
          <Reveal className="mx-auto max-w-2xl text-center">
            ...
            <p className="lead mt-4">
              Teknologien er ikke det svære. Det svære er at få den til at skabe værdi i en travl
              hverdag. Det er dér, vi kommer ind.
            </p>
          </Reveal>
      ...
      <section className="section bg-muted/40">
        <div className="container">
          <Reveal className="max-w-2xl">
            ...
            <p className="lead mt-4">Vi møder jer dér, hvor I er — fra første afklaring til løsninger i drift.</p>
          </Reveal>
            ...
            {SERVICES.map((s, i) => (
              <Reveal key={s.title} delay={i * 90}>
                <Card className="flex h-full flex-col border-border p-7">
                  ...
                  <Link to="/ydelser" className="mt-6 inline-flex items-center gap-1 text-sm font-semibold text-teal hover:underline">
          ...
          <Reveal className="mt-16 rounded-2xl border border-border bg-background p-7 md:p-10">
            <div className="max-w-xl">
              <h3 className="h3">AI i afdelingen</h3>
              <p className="mt-3 text-muted-foreground">
                Hvor i jeres organisation skal vi kigge først? Vi har konkrete forløb til hver afdeling.
              </p>
            </div>
            <ul className="mt-6 flex flex-wrap gap-2.5">
              {DEPARTMENTS.map((d) => (
                <li key={d.label}>
                  <Link to={d.href} className="inline-flex items-center rounded-full border border-border bg-secondary/60 px-4 py-2 text-sm font-medium text-foreground transition-colors hover:border-teal hover:text-teal">
                    {d.label}
                  </Link>
                </li>
              ))}
      ...
      <section className="section">
        <div className="container">
          ...
          <div className="mt-10">
            <Link to="/cases" className="inline-flex items-center gap-1 font-semibold text-teal hover:underline">
              Se flere cases <ArrowRight className="h-4 w-4" aria-hidden />
            </Link>
          </div>
        </div>
      </section>
        ...
        <div className="container grid items-center gap-10 md:grid-cols-12">
          <Reveal className="md:col-span-5">
            <div className="overflow-hidden rounded-2xl border border-border clip-slope-t">
              <img src={jacob} alt="Jacob Dalhoff" className="aspect-[4/5] w-full object-cover" width={933} height={1166} loading="lazy" />
            </div>
          </Reveal>
          <Reveal delay={120} className="md:col-span-7">
            ...
            <p className="mt-4 text-lg text-muted-foreground">
              Vi sidder i Dansk Standards AI-udvalg og følger EU's AI Act tæt, så I kan bruge AI
              ansvarligt og trygt.
            </p>
            <div className="mt-7 flex flex-wrap items-center gap-3">
              <Button asChild size="lg">
                <Link to="/book">Tag en snak med Jacob <ArrowRight className="h-4 w-4" /></Link>
              </Button>
              <Button asChild variant="outline" size="lg">
                <Link to="/om-os">Mere om os</Link>
              </Button>
            </div>
          </Reveal>
        </div>
        ...
        <div className="container">
          <Card className="mx-auto max-w-3xl border-border p-8 text-center md:p-12">
            ...
            <p className="mx-auto mt-3 max-w-prose text-muted-foreground">
              Konkrete tips, værktøjer og overblik over AI for danske virksomheder — i et sprog,
              alle kan forstå. Ingen spam.
            </p>
            <form onSubmit={onSubscribe} className="mx-auto mt-6 max-w-md text-left">
              <div className="flex flex-col gap-2 sm:flex-row">
                ...
                <input id="nl-email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="din@email.dk"
              ...
              <label className="mt-3 flex items-start gap-2 text-sm text-muted-foreground">
                <input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} className="mt-1 h-4 w-4 accent-teal" />
